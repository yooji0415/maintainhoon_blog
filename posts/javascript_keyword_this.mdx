...
> **어떻게 이렇게 동작하는 것일까?** > **메서드는 바인딩된 함수**이다. 위 person 객체의 getName 프로퍼티가 가리키는 함수 객체는 person 객체에 포함된 것이 아니라 독립적으로 존재하는 별도의 객체이다. getName 프로퍼티가 함수 객체를 가리키고 있을 뿐이다.
...
**apply, call, bind 메서드는 Function.prototype 의 메서드**이다.
이 말은 **모든 함수가 상속받아서 사용할 수 있는 메서드**라는 뜻이다.

**apply, call 메서드는 this 로 사용할 객체와 인수 리스트를 인수로 전달받아 함수를 호출**한다.
